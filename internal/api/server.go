...
func (s *Server) register(testAPI bool) {
	...
	s.mux.Handle("GET /api/v1/blocks/{id}/occupancy-sources", s.requireAuth(http.HandlerFunc(s.getBlockOccupancySources)))
	...
	s.mux.Handle("POST /api/v1/layout/validate", s.requireAuth(http.HandlerFunc(s.validateLayout)))
